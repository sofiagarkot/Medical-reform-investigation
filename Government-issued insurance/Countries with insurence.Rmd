...
for(i in seq(1, length(successful))) {
  ...
  for (j in seq(0, -6,-1)){
    ...
    # print(country)
    # print(year+j)
    # Тут просто подивись чим би воно мало заповнювати а чим -- заповнює насправді!
    # print(gdp[which(gdp$LOCATION==country & gdp$TIME == year+j & gdp$SUBJECT == "TOT" & gdp$MEASURE == "USD_CAP"), ])
    data[c, 4] =  gdp[which(gdp$LOCATION==country & gdp$TIME == year+j & gdp$SUBJECT == "TOT" & gdp$MEASURE == "USD_CAP"),7]
    # print(data[c,4])
    # print(inflation[which(inflation$LOCATION==country & inflation$TIME == year+j & inflation$SUBJECT == "TOT" & inflation$MEASURE == "AGRWTH"), ])
...
for(i in seq(1, length(unsuccessful))) {
  ...
  for (j in seq(0, -6,-1)){
    ...
    # print(gdp[which(gdp$LOCATION=="EST" & gdp$TIME == 1996 & gdp$MEASURE == "USD_CAP"), 7])
    #print(data[c,4])
    #print(inflation[which(inflation$LOCATION==country & inflation$TIME == year+j & inflation$SUBJECT == "TOT" & inflation$MEASURE == "AGRWTH"), ])
    data[c, 5] = inflation[which(inflation$LOCATION==country & inflation$TIME == year+j & inflation$SUBJECT == "TOT" & inflation$MEASURE == "AGRWTH"), 7]
    data[c, 6] = ifelse(length(which(spending$LOCATION == country & spending$TIME == year+j & spending$MEASURE == "PC_GDP"  & spending$SUBJECT =="TOT")) == 0, NA, spending[which(spending$LOCATION == country & spending$TIME == year+j & spending$MEASURE == "PC_GDP" & spending$SUBJECT =="TOT"), 7])
...
all.years = c(introduced.years,unsuccessflul.years)
```
...
```
We see the strong evidence that there is exponential trending in this time series data. that is why I will detrend it.
```{r}
...
```
An then trying to plot detrended data
```{r}
...
```
Here we do not obseerve some tendency for GDP to grow or fall before the introduction of the reform.
### Another metrics
...
```
So we see that there is no linear trend. 
...
```
```{r}
data$detrended.unemployment = NA
detrended.row.index = 1
for (c in all.countries){
  trend.country = lm(data[["unemloyment"]][which(data$country == c)]~data[["year"]][which(data$country == c)])
  detrended.country = data[["unemloyment"]][which(data$country == c)] - predict(trend.country)
  # print(detrended.country)
  for (i in seq(1,7,1)){
    data[detrended.row.index, 10] = detrended.country[i]
    detrended.row.index = detrended.row.index + 1
}
}
data
...
```
Here we also find no dependancy on time.
...
for(country in all.countries) {
  print(country)
    ...
    regressors[data.pointer, 6] = weights %*% data[which(data$country == country), 10]
...
summary(logit)
```
We see that none of them has at least some significant influence on the responce variable. That is why building a prediction model I will exclude them.
...
```
```{r}
fit = glm(indicator ~ deficit, data=data, family=binomial)
newdat <- data.Ukraine
newdat$vs = predict(fit, newdata=newdat, type="response")
plot(indicator ~ deficit, data=data, col="red4")
lines(indicator ~ deficit, newdat, col="green4", lwd=2)
```
...
Data collected at :https://www.ceicdata.com/en/indicator/ukraine/government-debt--of-nominal-gdp , https://www.theglobaleconomy.com/Ukraine/household_consumption/ 
```{r}
